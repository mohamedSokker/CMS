      ...
      item.category === cat && (
        ...
        >
          ...
          <div className="w-full flex flex-row justify-start items-center gap-2">
            ...
            item.category === cat ? (
              <input
                className="w-[70%] text-[10px] p-1 text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-gray-900"
                type="text"
            ...
            {item?.inputType === "select" &&
              ...
              item.category === cat && (
                <select
                  className=" w-[70%] text-[10px] p-1 dark:text-white dark:bg-gray-800"
                  value={data?.el?.[selectedItem]?.[item?.name]}
                  onChange={(e) => handleChange(e, item)}
                >
                  ...
                  {tablesData &&
                    Object.keys(tablesData)?.map((el, idx) => (
                      <div key={idx} className="w-full flex flex-row gap-2">
                        ...
                        />
                        <p className="text-[10px] dark:text-white">{el}</p>
                      </div>
                    ))}
                </div>
              )}
            {item.category === cat && (
              <p className="text-[10px] dark:text-white">{item?.unit}</p>
            )}
          </div>
        </div>
      )
