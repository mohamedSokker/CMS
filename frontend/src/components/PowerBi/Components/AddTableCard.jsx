...
const AddTableCard = ({
  ...
  const { DBdata, loading, getTableData, tableData } = useDataContext();

  const { setIsItemUnChecked, setIsSortChecked } = useInitContext();
  ...
  const handleSave = async () => {
    ...
    isChoose?.map(async (item) => {
      const data =
        tablesData?.[item]?.data && tablesData?.[item]?.data?.length > 0
      ...
      if (data.length > 0) {
        ...
        setIsSortChecked((prev) => ({ ...prev, [item]: ["ID"] }));
      }
    });
    setIsDataReady(true);
    setTimeout(() => {
