    ...
    >
      ...
      >
        <div className="w-full h-full flex flex-row justify-start items-start px-1">
          ...
          />
          <div
            className="w-[200px] h-full bg-gray-300 dark:bg-gray-800 overflow-x-scroll overflow-y-scroll flex flex-col gap-2 p-2"
            style={{ scrollbarWidth: "none" }}
          >
            {loading ? (
              ...
              DBdata?.map((item, i) => (
                <div
                  key={i}
                  className="w-full flex flex-row gap-2 justify-start items-center bg-white dark:bg-gray-900 dark:text-white rounded-[4px] p-1 text-[12px] "
                >
                  <input
                    type="checkbox"
