...
const NonExistingColumns = ({
  ...
  const handleFinish = async () => {
    try {
      ...
      //   Exist: data?.Exist,
      // });
      // const url = `/api/v3/CustomDataEntryCreateTable/${data?.ID}`;
      // await axiosPrivate(url, { method: "PUT", data: JSON.stringify(data) });

      const addURL = `/api/v3/ManageDataEntry/${data?.ID}`;
      setMessage(`Saving Data...`);
