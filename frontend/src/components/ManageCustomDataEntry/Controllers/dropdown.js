  ...
  useEffect(() => {
    const getFromData = async () => {
      try {
        ...
        const URLs = [
          `/api/v3/getTableData?table=${data?.Fields?.[name]?.[label]}`,
        ];
        ...
        setData((prev) => ({
          ...
          Fields: {
            ...
            [name]: {
              ...
              URL: `/api/v3/${data?.Fields?.[name]?.[label]}`,
            },
          },
        }));
        setLoading(false);
    ...
    if (label && label === "From" && found === true) getFromData();
    if (label && val && label === "Table") getTablesData();
  }, [label]);
